...
function insertLocalHashDeclaration(
  text: string,
  ...
  if (insertPoint === -1) {
    // @ts-ignore
    for (const match of text.matchAll(
      /local\s+[a-zA-Z_][0-9a-zA-Z_]*\s*=\s*require(\(|\s)[^\r\n]*/g
    )) {
      insertPoint = match.index + match[0].length;
      newlinesBefore = 2;
      newlinesAfter = 0;
...
function insertModuleHashDeclaration(
  text: string,
  hashIdentifier: string,
  string: string
): Insertion | null {
...
function insertModuleRequire(
  ...
  if (insertPoint === -1) {
    ...
    for (const match of text.matchAll(
      ...
      if (match[1] === moduleRequireBinding) {
        return null;
      }
      insertPoint = match.index + match[0].length;
      newlinesBefore = 1;
...
function replaceInDocument(
  ...
  while (true) {
    ...
    const match = text
      .substr(0, index)
      .match(new RegExp(`(local ${to} = )?hash\\(\\s*$`));
...
function hashDeclarationAlreadyExists(
  documentText: string,
  hashIdentifier: string,
  moduleText?: string
) {
  if (moduleText) {
    return new RegExp(`(^|\\n)M\\.${hashIdentifier}\\s*=\\s*hash\\(`).test(
      moduleText
    );
  }
  return new RegExp(`(^|\\n)local\\s+${hashIdentifier}\\s*=\\s*hash\\(`).test(
    documentText
  );
}
...
export function refactorHashes(
  selections: string[],
  documentText: string,
  moduleText: string,
  options: {
  ...
  selections.forEach((selection) => {
    ...
    if (hashes.find((item) => item.hash === hash)) {
      return;
    }
    ...
    const hashIdentifier =
      prefix + (capitalise ? identifier.toUpperCase() : identifier);

    const shouldDeclare = !hashDeclarationAlreadyExists(
      documentText,
      hashIdentifier,
      moduleText
    );
    ...
    ({
      ...
      if (shouldDeclare) {
        if (modulePath) {
          addEdit(
            documentEdits,
            insertModuleRequire(documentText, modulePath, moduleRequireBinding)
          );
        } else {
          addEdit(
            documentEdits,
            insertLocalHashDeclaration(
              documentText,
              hashIdentifier,
              stringDoubleQuoted
            )
      ...
      const identifier = modulePath
        ? `${moduleRequireBinding}.${hashIdentifier}`
        : hashIdentifier;
      replaceInDocument(
        documentText,
        documentEdits,
