...
function applyEdit(document: TextDocument, workspaceEdit: WorkspaceEdit, edit: Edit | null) {
  if (!edit) return
  switch (edit.type) {
    case 'insert':
      if (edit.text) {
        workspaceEdit.insert(document.uri, document.positionAt(edit.offset), edit.text)
      }
      break
    case 'replace':
      if (edit.text) {
        workspaceEdit.replace(document.uri, new Range(
...
function applyEditToNewDocument(uri: Uri, workspaceEdit: WorkspaceEdit, edit: Edit | null) {
  if (!edit) return
  switch (edit.type) {
    case 'insert':
      if (edit.text) {
...
export default function registerRefactorHashCommand(context: ExtensionContext) {
    ...
    const { documentEdits, moduleEdits } = refactorHashes(selections, documentText, moduleText, {
      prefix, capitalise, modulePath, moduleRequireBinding,
    })

    if (documentEdits.length === 0 && moduleEdits.length === 0) { return }
